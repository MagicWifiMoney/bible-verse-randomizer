/**
 * Sprint 2 Batch 2: Generate 970 High-Value Verse Pages
 * 
 * Selection criteria:
 * - Continue from Batch 1 - next 970 verses
 * - Same high-priority books, different verse ranges
 * 
 * Generated content:
 * - Context (300 words)
 * - Meaning (400 words)
...
const CHECKPOINT_FILE = path.join(__dirname, '../data/sprint2-batch2-checkpoint.json');
const PROGRESS_FILE = path.join(__dirname, '../data/sprint2-batch2-progress.json');
...
// Same high-priority books as Batch 1
...
async function selectTopVerses() {
  console.log('📋 Selecting next 970 verses (Batch 2)...\n');
  
  const verses = [];
  
  // Get verses from high-priority books
  // Batch 1 took verses 1-100 from top 10 books (970 total)
  // Batch 2 takes verses 101-200 from those same books, plus extras from remaining books
  
  for (let bookIndex = 0; bookIndex < HIGH_PRIORITY_BOOKS.length; bookIndex++) {
    if (verses.length >= TARGET_VERSES) break;
    
    const book = HIGH_PRIORITY_BOOKS[bookIndex];
    const offset = bookIndex < 10 ? 100 : 0; // First 10 books skip first 100 verses
    const limit = bookIndex < 10 ? 100 : 150; // Get more from other books
    ...
    console.log(`   ${book}: ${result.rows.length} verses (offset ${offset})`);
...
async function generateBatch() {
  console.log('🚀 Sprint 2 Batch 2: Generating 970 verse pages\n');
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(`✅ SPRINT 2 BATCH 2 COMPLETE`);
  console.log(`${'='.repeat(60)}\n`);
  ...
  // Save final stats
  fs.writeFileSync(
    path.join(__dirname, '../data/sprint2-batch2-final.json'),
