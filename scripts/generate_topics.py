...
from main import keyword_research, get_keyword_suggestions, get_keyword_ideas
...
def gather_topics() -> List[Dict[str, Any]]:
    ...
    for category, topics in SEED_TOPICS.items():
        ...
        for topic in topics:
            ...
            try:
                ...
                # Extract search volume from overview
                search_volume = 0
                competition = 0
                
                if result.get("overview"):
                    # Results are stored in files, load the latest
                    overview_data = result["overview"]
                    if isinstance(overview_data, dict) and overview_data.get("result"):
                        for item in overview_data["result"]:
                ...
                if search_volume >= 100:
                    ...
                    print(f"      ✓ Added: {search_volume} searches/mo")
                else:
    ...
    for base in base_queries[:1]:  # Just use first one to avoid too many API calls
        try:
            print(f"\n🔍 Getting suggestions for: {base}")
            ...
            if isinstance(suggestions, dict) and suggestions.get("result"):
